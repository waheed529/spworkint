...
public class JdbcCalendarUserDao implements CalendarUserDao {
	...
	public List<CalendarUser> findUsersByEmail(String email) {
		...
		return jdbcOperations.query(CALENDAR_USER_QUERY + "email like ? order by id", CALENDAR_USER_MAPPER,
				email + "%");
	}
	...
	public int createUser(final CalendarUser userToAdd) {
		...
		if (userToAdd.getId() != null) {
			throw new IllegalArgumentException(
					"userToAdd.getId() must be null when creating a " + CalendarUser.class.getName());
		}
	...
	/**
	 * Create a new RowMapper that resolves {@link CalendarUser}'s given a column
	 * label prefix. By allowing the prefix to be specified we can reuse the same
	 * {@link RowMapper} for joins in other tables.
	 *
	 * @author Rob Winch
	 *
	 */
